...
const props = defineProps({
    ...
    customerTypes: Array, // Daftar tipe customer untuk dropdown
});
...
const submitForm = async () => {
    try {
        ...
        if (response.data.success) {
            emit('customerAdded', response.data.data); // Emit customer baru
            emit('close'); // Tutup modal
            form.reset(); // Reset form setelah sukses
            alert(response.data.message); // Ganti dengan notifikasi yang lebih baik
        } else {
            alert(response.data.message || 'Gagal menambahkan customer.'); // Ganti dengan notifikasi yang lebih baik
        }
    } catch (error) {
        if (error.response && error.response.data && error.response.data.errors) {
            ...
            alert('Terjadi kesalahan saat menambahkan customer.'); // Ganti dengan notifikasi yang lebih baik
        }
            ...
            <div v-if="show" class="fixed inset-0 z-40 flex items-center justify-center p-4">
                ...
                <div
                    ...
                    <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                        <h3 class="text-lg font-medium text-gray-900">Tambah Konsumen Cepat</h3>
...
<style scoped>
/* Transisi Modal (sama seperti CustomerFormModal.vue) */

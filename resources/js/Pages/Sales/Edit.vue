...
const props = defineProps({
    sale: Object,       // Data penjualan yang akan diedit
...
const form = useForm({
    ...
    paid_at: props.sale.paid_at, // paid_at dari data existing
    is_paid: !!props.sale.paid_at, // Inisialisasi toggle berdasarkan paid_at
    details: props.sale.details.map(detail => ({ // Mapping detail yang sudah ada
        id: detail.id, // Pertahankan ID untuk kemungkinan update spesifik (walaupun backend kita delete/recreate)
...
const selectedCustomerDiscount = computed(() => {
    ...
    return customer ? customer.customer_type_discount : 0;
});
...
const submitForm = () => {
    if (!canEdit.value) {
        alert('Anda tidak memiliki izin untuk mengedit penjualan.');
        return;
    }
    ...
    form.put(route('penjualan.update', props.sale.id), {
        onSuccess: () => {
            // Setelah sukses, mungkin tidak perlu reset form karena data sudah di-update
...
<template>
    ...
    <Head :title="`Edit Penjualan: ${sale.code}`" />

    <div>
        <!-- <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Penjualan: {{ sale.code }}</h2>
        </template> -->
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        ...
                        <form @submit.prevent="submitForm" v-if="canEdit">
                            ...
                            <div class="text-right mt-8">
                                <h4 class="text-2xl font-bold text-gray-800">Total: {{
                                    ...
                                    style: 'currency', currency: 'IDR' }) }}</h4>
                            </div>
                        ...
                        <div v-else
                            ...
                            role="alert">
                            Anda tidak memiliki izin untuk mengedit penjualan ini.
                        </div>
                    </div>
                </div>
            </div>
    ...
    <AddCustomerModal :show="showAddCustomerModal" :customerTypes="props.customers.reduce((acc, curr) => {
            if (curr.customer_type_id && curr.customer_type_discount !== undefined) {
                if (!acc.find(item => item.id === curr.customer_type_id)) {
                    // This is a simplified way to get customer types from customer prop.
                    // Ideally, pass customerTypes directly from controller.
                    // For now, assuming customerTypes are passed as a separate prop or extracted here.
                    // For better handling, ensure you pass a dedicated `customerTypes` prop from backend.
                    acc.push({ id: curr.customer_type_id, name: 'Tipe ' + curr.customer_type_id }); // Dummy name, replace if actual name is available
                }
            }
            return acc;

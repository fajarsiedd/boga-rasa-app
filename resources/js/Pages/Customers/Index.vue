...
const fetchData = () => {
    ...
    router.get(route('konsumen.index'), params, {
        preserveScroll: true,
        preserveState: true
    });
}
...
const resetFilters = () => {
    ...
    router.get(route('konsumen.index'), {});
}
</script>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-700">
                    ...
                    <div v-if="customers.length > 0 || isFiltered"
                        ...
                        <div class="flex flex-row gap-2">
                            <form @submit.prevent="fetchData">
                                <div
                                    ...
                                    <button type="submit">
                                        <IconSearch
                                            class="mr-2 text-gray-400 group-focus-within:text-green-700 group-hover:text-green-700 hover:cursor-pointer transition-colors duration-200"
                                            stroke="1.5" />
                                    </button>
                                </div>
                            </form>
                    ...
                    <div v-if="customers.length === 0"
                        ...
                        <p class="font-semibold mb-2">{{ isFiltered ? 'Data tidak ditemukan' : 'Belum ada data konsumen'
                        }}
                        </p>
                        ...
                        <p v-else="!isFiltered" class="text-sm text-center text-gray-500 mb-4">Silakan gunakan kata
                            kunci lain.</p>
                        <Link v-if="canCreate && !isFiltered" :href="route('konsumen.create')"
                            class="inline-flex items-center px-4 py-2 bg-green-700 border border-transparent rounded-md font-semibold text-sm text-white hover:bg-green-800 focus:outline-none focus:border-green-800 focus:ring ring-green-300 disabled:opacity-25 transition ease-in-out duration-150">
                        <IconPlus class="mr-2" size="20" />
                        <span>Tambah Konsumen Baru</span>
                        </Link>

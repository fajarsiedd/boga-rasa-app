...
const form = useForm({
    ...
    discount: props.customerType.discount,
});

const submit = () => {
    form.put(route('tipe-konsumen.update', props.customerType.id), {
        onSuccess: () => { },
        onError: () => { },
    });
};
...
<template>
    <AuthenticatedLayout>
        ...
        <Head title="Edit Tipe Konsumen Baru" />

        <template #header>
            <h2 class="font-semibold text-lg text-gray-700 leading-tight">Edit Tipe Konsumen - {{ customerType.name }}
            </h2>
        </template>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-700">
                        <form @submit.prevent="submit">
                            ...
                            <div class="mb-4">
                                <label for="discount" class="block text-sm font-medium text-gray-700">Diskon
                                    <span class="text-red-500">*</span>
                                </label>
                                <input id="discount" type="text"
                                    class="mt-1 block w-full rounded-md px-2 bg-white h-10 border border-gray-300 focus:border-none focus:outline-none focus:ring-2 focus:ring-green-700"
                                    v-model="form.discount" required />
                                <div v-if="form.errors.discount" class="text-red-600 text-sm mt-1">{{
